...
private class PropertyReferenceLowering(val context: JvmBackendContext) : IrElementTransformerVoidWithContext(), FileLoweringPass {
    ...
    private fun createReflectedKProperty(expression: IrCallableReference<*>): IrExpression {
        val optimized = context.state.generateOptimizedCallableReferenceSuperClasses
        val boundReceiver = expression.dispatchReceiver ?: expression.extensionReceiver
        if (boundReceiver != null && !optimized) {
        ...
        return context.createJvmIrBuilder(currentScope!!.scope.scopeOwnerSymbol, expression.startOffset, expression.endOffset).run {
            val arity = when {
                ...
                optimized -> 4 // (jClass, name, desc, flags)
            ...
            val instance = irCall(referenceKind.implSymbol.constructors.single { it.owner.valueParameters.size == arity }).apply {
                fillReflectedPropertyArguments(this, expression, optimized, boundReceiver)
            }
    ...
    private fun JvmIrBuilder.fillReflectedPropertyArguments(
        ...
        call.putValueArgument(index++, if (optimized) kClassToJavaClass(calculateOwnerKClass(container)) else calculateOwner(container))
            ...
            body = context.createJvmIrBuilder(symbol).run {
                irBlockBody(startOffset, endOffset) {
                    +irDelegatingConstructorCall(superConstructor).apply {
                        fillReflectedPropertyArguments(this, expression, optimized = true, receiverParameter?.let(::irGet))
                    }
