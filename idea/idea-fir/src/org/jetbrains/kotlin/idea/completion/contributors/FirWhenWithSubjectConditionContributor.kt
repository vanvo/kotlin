...
import org.jetbrains.kotlin.idea.completion.contributors.helpers.createStartTypeArgumentsList
...
internal class FirWhenWithSubjectConditionContributor(
    ...
    private fun KtAnalysisSession.addNull(type: KtType?) {
    ...
    private fun KtAnalysisSession.isPrefixNeeded(classifier: KtClassifierSymbol): Boolean {
        return when (classifier) {
            ...
            is KtTypeAliasSymbol -> (classifier.expendedType as? KtClassType)?.classSymbol?.let { isPrefixNeeded(it) } == true
    ...
    private fun KtAnalysisSession.getHandledClassIds(conditions: List<KtWhenCondition>): List<ClassId> =
    ...
    private fun addElseBranch(whenCondition: KtWhenCondition) {
    ...
    private fun KtAnalysisSession.completeEnumEntries(
        ...
        val handledCasesNames = conditions.mapNotNull { condition ->
    ...
    private fun KtAnalysisSession.addLookupElement(
        ...
        LookupElementBuilder.create(lookupObject, getIsPrefix(isPrefixNeeded) + lookupString)
            ...
            .withTailText(createStartTypeArgumentsList(typeArgumentsCount), /*grayed*/true)
...
private object WhenConditionInsertionHandler : InsertionHandlerBase<WhenConditionLookupObject>(WhenConditionLookupObject::class) {
    override fun handleInsert(context: InsertionContext, item: LookupElement, ktFile: KtFile, lookupObject: WhenConditionLookupObject) {
        context.handleConflictsInName(lookupObject, ktFile)
    ...
    private fun InsertionContext.handleConflictsInName(
        lookupObject: WhenConditionLookupObject,
        ktFile: KtFile
    ) {
