...
import org.jetbrains.kotlin.test.utils.TeamcityChecker.isTeamCityBuild
import org.jetbrains.kotlin.test.utils.removeDirectiveFromFile
import org.jetbrains.kotlin.test.utils.TeamcityChecker
...
class IdeTestIgnoreHandler(testServices: TestServices) : AfterAnalysisChecker(testServices) {
    ...
    override fun check(failedAssertions: List<Throwable>) {
        ...
        if (!TeamcityChecker.isTeamCityBuild) {
            testDataFile.removeDirectiveFromFile(FirDiagnosticsDirectives.FIR_IDE_IGNORE)
        }

        val message = if (TeamcityChecker.isTeamCityBuild) {
            "Please remove // ${FirDiagnosticsDirectives.FIR_IDE_IGNORE} from the test source"
        } else {
            "Removed // ${FirDiagnosticsDirectives.FIR_IDE_IGNORE} from the test source"
        }
    ...
    private fun isFirIdeIgnoreDirectivePresent(): Boolean {
        val moduleStructure = testServices.moduleStructure
        return FirDiagnosticsDirectives.FIR_IDE_IGNORE in moduleStructure.allDirectives
    }
}
