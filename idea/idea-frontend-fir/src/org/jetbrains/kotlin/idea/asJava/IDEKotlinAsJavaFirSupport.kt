...
class IDEKotlinAsJavaFirSupport(private val project: Project) : KotlinAsJavaSupport() {
    ...
    override fun getFacadeClasses(facadeFqName: FqName, scope: GlobalSearchScope): Collection<PsiClass> =
        ...
        findFilesForFacade(facadeFqName, scope).ifNotEmpty {
            listOfNotNull(getOrCreateFirLightFacade(this, facadeFqName))
        } ?: emptyList()
    ...
    override fun findFilesForFacade(facadeFqName: FqName, scope: GlobalSearchScope): List<KtFile> {
        if (facadeFqName.shortNameOrSpecial().isSpecial) return emptyList()
        return project.createDeclarationProvider(scope)
            .getFacadeFilesInPackage(facadeFqName.parent())
