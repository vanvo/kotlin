...
object ClasspathChangesComputer {
    ...
    fun getChanges(current: ClasspathSnapshot, previous: ClasspathSnapshot): ClasspathChanges {
        ...
        return when (ClasspathChangesCollector.collect(current, previous, changesCollector)) {
            Success -> {
...
private object ClasspathChangesCollector {

    fun collect(current: ClasspathSnapshot, previous: ClasspathSnapshot, changesCollector: ChangesCollector): ChangesCollectorResult {
        ...
        for (index in current.classpathEntrySnapshots.indices) {
            val result = ClasspathEntryChangesCollector.collect(
...
private object ClasspathEntryChangesCollector {
    ...
    fun collect(
        ...
        for (key in current.classSnapshots.keys) {
            ...
            val result = ClassChangesCollector.collect(currentSnapshot, previousSnapshot, changesCollector)
            if (result is Failure) {
                return result
            }
        }
...
private object ClassChangesCollector {

    fun collect(current: ClassSnapshot, previous: ClassSnapshot, changesCollector: ChangesCollector): ChangesCollectorResult {
